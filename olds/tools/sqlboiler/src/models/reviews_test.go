...
func testReviewsExists(t *testing.T) {
	...
	e, err := ReviewExists(ctx, tx, o.ContentID, o.SellerID)
	if err != nil {
...
func testReviewsFind(t *testing.T) {
	...
	reviewFound, err := FindReview(ctx, tx, o.ContentID, o.SellerID)
	if err != nil {
		t.Error(err)
	}
...
func testReviewToOneContentUsingContent(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadContent(ctx, tx, false, (*[]*Review)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testReviewToOneSetOpContentUsingContent(t *testing.T) {
	var err error
	...
	for i, x := range []*Content{&b, &c} {
		...
		if exists, err := ReviewExists(ctx, tx, a.ContentID, a.SellerID); err != nil {
			t.Fatal(err)
		} else if !exists {
...
var (
	reviewDBTypes = map[string]string{`ContentID`: `bigint`, `SellerID`: `bigint`, `ReviewsID`: `bigint`, `Result`: `integer`}
	_             = bytes.MinRead
)
