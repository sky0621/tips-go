...
func testCustomerToManyUserTodos(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.UserTodos().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadUserTodos(ctx, tx, false, (*[]*Customer)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserTodos); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UserTodos = nil
	if err = a.L.LoadUserTodos(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserTodos); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testCustomerToManyAddOpUserTodos(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddUserTodos(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.UserTodos[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UserTodos[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.UserTodos().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
