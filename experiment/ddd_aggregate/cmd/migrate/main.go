...
func loadMySQLConfig() (mysqlConfig, error) {
	port, err := parsePort(getEnv("SQLDEF_MYSQL_PORT", strconv.Itoa(defaultPort)))
	if err != nil {
		return mysqlConfig{}, err
	}

	cfg := mysqlConfig{
		Host:     getEnv("SQLDEF_MYSQL_HOST", defaultHost),
		Port:     port,
		User:     getEnv("SQLDEF_MYSQL_USER", defaultUser),
		Password: getEnv("SQLDEF_MYSQL_PASSWORD", defaultPassword),
		Database: getEnv("SQLDEF_MYSQL_DATABASE", defaultDatabase),
